...
async function fetchBooks(resetResults = true) {
  ...
  try {
    ...
    try {
      ...
      newBooks = (data.items || []).map(normalizeBookData);
      totalResults = data.totalItems || 0;
    } catch (googleError) {
      console.warn("Google Books unavailable, using Open Library fallback:", googleError);
      const fallbackData = await fetchOpenLibraryBooks();
      newBooks = fallbackData.books;
      totalResults = fallbackData.totalResults;
      showToast("Google Books is temporarily busy. Results loaded from Open Library.");
    }
    ...
    showErrorState("The books could not be loaded right now. Please try again.");
  } finally {
...
async function fetchOpenLibraryBooks() {
  const page = Math.floor(state.startIndex / 20) + 1;
  const parameters = new URLSearchParams({
    q: state.currentQuery,
    page: String(page),
    limit: "20",
    fields:
      "key,title,author_name,first_publish_year,publisher,cover_i,isbn,language,subject,number_of_pages_median",
  });
...
function normalizeOpenLibraryBook(item) {
  ...
  return {
    id: item.key || `open-library-${item.title}-${item.first_publish_year || "unknown"}`,
    ...
    description: "Book information provided by Open Library.",
